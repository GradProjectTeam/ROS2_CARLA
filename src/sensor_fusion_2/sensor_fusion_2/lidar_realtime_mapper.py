...
class LidarRealtimeMapper(Node):
    ...
    def __init__(self):
        ...
        # Add parameter for map topic
        if not self.has_parameter('map_topic'):
            self.declare_parameter('map_topic', '/realtime_map')  # Default map topic
        
        # Add parameters for QoS settings
        if not self.has_parameter('use_reliable_qos'):
            self.declare_parameter('use_reliable_qos', True)  # Use reliable QoS by default
        if not self.has_parameter('use_transient_local_durability'):
            self.declare_parameter('use_transient_local_durability', True)  # Use transient local durability by default
        
        # Add parameter for integrating radar data
        if not self.has_parameter('integrate_radar_data'):
            self.declare_parameter('integrate_radar_data', True)  # Whether to integrate radar data
        if not self.has_parameter('radar_map_topic'):
            self.declare_parameter('radar_map_topic', '/radar/map')  # Radar map topic
        if not self.has_parameter('radar_weight'):
            self.declare_parameter('radar_weight', 0.4)  # Weight for radar data in integration
        ...
        # Get radar integration parameters
        self.integrate_radar_data = self.get_parameter('integrate_radar_data').value
        ...
        # Add transient local durability if enabled
        if self.use_transient_local_durability:
            reliable_qos.durability = QoSDurabilityPolicy.TRANSIENT_LOCAL
        
        # Publishers - main realtime map and visualization
        self.map_publisher = self.create_publisher(
            OccupancyGrid, 
            self.map_topic,  # Use the parameter value instead of hardcoded topic
            reliable_qos
        )
        ...
        # Add subscriber for radar map if integration is enabled
        if self.integrate_radar_data:
            # Create QoS profile for radar map subscription
            ...
            self.get_logger().info(f"Subscribing to radar map on topic: {self.radar_map_topic}")
    ...
    def process_data(self):
        ...
        # Process radar data if available and integration is enabled
        if self.integrate_radar_data:
            with self.radar_lock:
                radar_available = self.radar_map_data is not None and self.new_radar_data
                if radar_available:
                    self.update_from_radar_map()
                    self.new_radar_data = False
    ...
    def radar_map_callback(self, msg):
        """Process incoming radar map data"""
        with self.radar_lock:
            ...
            self.new_radar_data = True
            self.get_logger().debug(f"Received radar map, size: {width}x{height}")
    
    def update_from_radar_map(self):
        """Update the realtime map using radar map data"""
        try:
            if self.radar_map_data is None or self.radar_map_info is None:
                return
                
            with self.map_lock:
                # Get radar map resolution and origin
                radar_resolution = self.radar_map_info.resolution
                radar_origin_x = self.radar_map_info.origin.position.x
                radar_origin_y = self.radar_map_info.origin.position.y
                
                # Get dimensions
                radar_width = self.radar_map_info.width
                radar_height = self.radar_map_info.height
                
                # Track cells that were updated
                cells_updated = 0
                
                # Create a temporary hit count map for radar data
                radar_hit_counts = np.zeros_like(self.hit_count_map)
                
                # Iterate through radar map cells
                for radar_y in range(radar_height):
                    for radar_x in range(radar_width):
                        # Get radar cell value
                        radar_value = self.radar_map_data[radar_y, radar_x]
                        
                        # Skip unknown or free cells
                        if radar_value <= 20:  # More lenient threshold to include more radar data
                            continue
                            
                        # Convert radar cell to world coordinates
                        world_x = radar_origin_x + radar_x * radar_resolution
                        world_y = radar_origin_y + radar_y * radar_resolution
                        
                        # Convert world coordinates to our map coordinates
                        map_x = int((world_x - self.map_origin_x) / self.map_resolution)
                        map_y = int((world_y - self.map_origin_y) / self.map_resolution)
                        
                        # Check if within our map bounds
                        if 0 <= map_x < self.map_width and 0 <= map_y < self.map_height:
                            # Convert radar value to a hit count equivalent - amplify for better visibility
                            # Higher radar values get significantly more weight
                            radar_hit_count = (radar_value / 100.0) * self.count_threshold * 3.0
                            
                            # Also update neighboring cells for better visibility
                            for dy in [-1, 0, 1]:
                                for dx in [-1, 0, 1]:
                                    nx, ny = map_x + dx, map_y + dy
                                    if 0 <= nx < self.map_width and 0 <= ny < self.map_height:
                                        # Center cell gets full value, neighbors get reduced value
                                        factor = 1.0 if (dx == 0 and dy == 0) else 0.5
                                        radar_hit_counts[ny, nx] = max(
                                            radar_hit_counts[ny, nx],
                                            radar_hit_count * factor
                                        )
                
                # Now update the actual map with the radar hit counts
                current_time = time.time()
                for y in range(self.map_height):
                    for x in range(self.map_width):
                        if radar_hit_counts[y, x] > 0:
                            # Blend radar data with existing lidar data
                            # Use radar_weight as the blending factor
                            blended_hit_count = max(
                                self.hit_count_map[y, x],
                                radar_hit_counts[y, x] * self.radar_weight
                            )
                            
                            # Only update if the radar data provides more evidence
                            if blended_hit_count > self.hit_count_map[y, x]:
                                self.hit_count_map[y, x] = blended_hit_count
                                
                                # Update map value based on hit count
                                new_value = int((blended_hit_count / self.count_threshold) * 100)
                                new_value = max(0, min(new_value, 100))
                                
                                if self.realtime_map[y, x] != new_value:
                                    self.realtime_map[y, x] = new_value
                                    self.last_update_time[y, x] = current_time
                                    self.map_changed = True
                                    cells_updated += 1
                
                if cells_updated > 0:
                    self.get_logger().info(f"Updated {cells_updated} cells from radar data")
                    
        except Exception as e:
            self.get_logger().error(f"Error updating from radar map: {str(e)}")
            import traceback
            self.get_logger().error(traceback.format_exc())
