...
def generate_launch_description():
    ...
    # Lane mapping parameters
    lane_projection_distance_arg = DeclareLaunchArgument(
        'lane_projection_distance',
        default_value='20.0',
        description='Distance to project lane markers in meters')
        
    lane_width_arg = DeclareLaunchArgument(
        'lane_width',
        default_value='3.5',
        description='Width of the lane in meters')

    # Map grid parameters
    map_resolution_arg = DeclareLaunchArgument(
        'map_resolution',
        default_value='0.1',
        description='Resolution of the map grid in meters per pixel')
        
    map_width_arg = DeclareLaunchArgument(
        'map_width',
        default_value='50.0',
        description='Width of the map grid in meters')
        
    map_height_arg = DeclareLaunchArgument(
        'map_height',
        default_value='50.0',
        description='Height of the map grid in meters')
    
    # Create static transform publisher for TF frames
    # This creates the necessary transformation tree: map -> base_link -> camera_link
    ...
    static_transform_base_to_camera = Node(
        ...
        arguments=['2.0', '0', '1.5', '0', '0', '0', 'base_link', 'camera_link']
    )
    ...
    imu_camera_lane_fusion_node = Node(
        ...
        parameters=[
            ...
            {'lane_projection_distance': LaunchConfiguration('lane_projection_distance')},
    ...
    # Create Lane to Map Mapper node
