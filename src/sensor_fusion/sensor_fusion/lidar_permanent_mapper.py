...
class LidarPermanentMapper(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('ground_threshold', 0.4)     # Increased to better filter ground points
        self.declare_parameter('max_points_to_process', 2000) # Limit points per update for performance
        self.declare_parameter('min_height', -0.5)         # Increased to exclude lower objects
        ...
        self.declare_parameter('update_threshold', 0.05)    # Significantly reduced threshold for rapid updates (0.0-1.0)
        ...
        self.declare_parameter('hit_weight', 0.99)           # Increased to be almost certain for hits
        self.declare_parameter('miss_weight', 0.01)          # Decreased for more definitive free space
        self.declare_parameter('prior_weight', 0.5)          # Keep as is for initial unknown state
        self.declare_parameter('count_threshold', 3.0)       # Reduced to require fewer observations
        ...
        # Service for saving just the binary black/white map
        self.save_binary_service = self.create_service(
            Trigger,
            'save_binary_map',
            self.save_binary_map_callback
        )
    ...
    def publish_map(self):
        ...
        with self.map_lock:
            ...
            # More binary classification with sharper boundaries
            # Unknown = -1, Free = 0, Occupied = 100
            ros_map = np.zeros(self.permanent_map.shape, dtype=np.int8)
            
            # Apply more aggressive thresholds for binary visualization
            ros_map[self.permanent_map < 45] = 0       # Free space (more cells classified as free)
            ros_map[self.permanent_map > 55] = 100     # Occupied (more cells classified as occupied)
            ros_map[(self.permanent_map >= 45) & (self.permanent_map <= 55)] = -1  # Minimal unknown area
            ...
            # Also publish visualization map which includes raw probability values
    ...
    def save_map(self):
        ...
        with self.map_lock:
            ...
            # Create binary map for visualization (strictly black and white)
            binary_map = np.zeros(self.permanent_map.shape, dtype=np.int8)
            
            # Apply more aggressive thresholds for binary visualization
            binary_map[self.permanent_map < 45] = 0       # Free space (white) - more cells classified as free
            binary_map[self.permanent_map > 55] = 100     # Occupied (black) - more cells classified as occupied
            binary_map[(self.permanent_map >= 45) & (self.permanent_map <= 55)] = -1  # Minimal unknown area (gray)
            
            # Also save as PNG for easy viewing - strictly black and white
            import matplotlib.pyplot as plt
            plt.figure(figsize=(12, 12))
            # Use binary colormap: white (0) for free, black (1) for occupied
            cmap = plt.cm.binary
            # We invert the map for display so 0=white (free) and 100=black (occupied)
            plt.imshow(binary_map, cmap=cmap, origin='lower', vmin=-1, vmax=100)
            plt.colorbar(label='Occupancy (White=Free, Black=Occupied)')
            plt.title(f'Binary Permanent Map {timestamp}')
            plt.savefig(os.path.join(self.map_save_dir, f"binary_map_{timestamp}.png"))
            
            # Also save the original grayscale version
            plt.figure(figsize=(12, 12))
            ...
            self.get_logger().info(f'Binary (black/white) map saved to {os.path.join(self.map_save_dir, f"binary_map_{timestamp}.png")}')
            self.last_save_time = time.time()
            self.map_changed = False
    ...
    def save_binary_map(self):
        """Save just a binary black and white version of the map (white=free, black=occupied)"""
        with self.map_lock:
            # Create filename with timestamp
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = os.path.join(self.map_save_dir, f"binary_map_{timestamp}.png")
            ...
            # Save as PNG - strictly black and white
            import matplotlib.pyplot as plt
            plt.figure(figsize=(12, 12))
            # Use binary colormap: white (0) for free, black (1) for occupied
            cmap = plt.cm.binary
            ...
            plt.imshow(binary_map, cmap=cmap, origin='lower', vmin=-1, vmax=100)
            plt.colorbar(label='Occupancy (White=Free, Black=Occupied)')
            plt.title(f'Binary Map {timestamp}')
            plt.savefig(filename)
            plt.close('all')
            
            self.get_logger().info(f'Binary (black/white) map saved to {filename}')
            
            return filename

    # Add a service to save just the binary map
    def save_binary_map_callback(self, request, response):
        """Service callback to save binary black/white map"""
        try:
            filename = self.save_binary_map()
            response.success = True
            response.message = f"Binary map saved to {filename}"
        except Exception as e:
            response.success = False
            response.message = f"Failed to save binary map: {str(e)}"
        return response
