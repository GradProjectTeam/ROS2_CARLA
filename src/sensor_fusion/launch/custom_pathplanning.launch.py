...
def generate_launch_description():
    ...
    obstacle_threshold_arg = DeclareLaunchArgument(
        'obstacle_threshold',
        default_value='0.55',  # Fixed: added decimal point (was '55')
        description='Threshold for considering a cell as an obstacle (0-1)'
    )
    
    max_iterations_arg = DeclareLaunchArgument(
        'max_iterations',
        default_value='3000',  # Reduced from 10000 to 3000 for faster planning
    ...
    hybrid_astar_node = Node(
        ...
        parameters=[{
            ...
            # Planning parameters 
            'obstacle_threshold': LaunchConfiguration('obstacle_threshold'),  # Use obstacle_threshold directly
            'publish_rate': 20.0,  # Keep high rate for faster updates
            ...
            'replan_on_move': True,  # Always enable replanning based on vehicle movement
            'position_change_threshold': 0.05,  # Even more sensitive - replan if moved more than 5cm
            'orientation_change_threshold': 0.02,  # Even more sensitive - replan if rotated more than ~1°
            
            # Force path replanning when obstacles are detected (new parameters)
            'detect_map_changes': True,  # Enable detection of map changes for replanning
            'force_replan_interval': 0.5,  # Force replan every 0.5 seconds regardless of movement
            'avoid_obstacles_weight': 10.0,  # Increased weight for obstacle avoidance
            
            # Topics and frames
            'map_topic': '/realtime_map',  # Direct string instead of LaunchConfiguration
            'vehicle_frame_id': 'base_link',
            'map_frame_id': 'map',
        }],
        output='screen'
    ...
    return LaunchDescription([
        # Add the pre-declared arguments first
        grid_size_arg,
        wheelbase_arg,
        ...
        DeclareLaunchArgument(
            'decay_rate',
            default_value='0.75',  # Faster decay (was 0.95), lower value means faster decay
            description='Rate at which old map data decays (0-1, lower = faster decay)'
        ),
        DeclareLaunchArgument(
            'update_threshold',
            default_value='0.001',  # Higher threshold for faster updates (was 0.0003)
            description='Threshold for map updates'
        ),
        DeclareLaunchArgument(
            'temporal_memory',
            default_value='0.05',  # Much shorter memory for faster decay (was 0.15)
            description='Duration to remember points (seconds)'
        ),
        DeclareLaunchArgument(
            'enable_map_reset',
            default_value='true',  # Keep map reset enabled
            description='Enable periodic map reset to clear old data'
        ),
        DeclareLaunchArgument(
            'map_reset_interval',
            default_value='3.0',  # Much more frequent map reset (was 8.0)
        ...
        DeclareLaunchArgument(
            ...
            default_value='20.0',  # Doubled from 10.0 to 20.0 Hz for faster updates
            description='Rate to publish planned paths (Hz)'
        ),
        DeclareLaunchArgument(
            'motion_resolution',
            default_value='5',  # Reduced from 10 to 5 for faster planning
            description='Number of steering angles to consider in motion model'
        ),
        DeclareLaunchArgument(
            'angle_resolution',
            default_value='18',  # Reduced from 36 to 18 for faster planning
            description='Angle discretization (18 = 20 degree resolution)'
        ),
        DeclareLaunchArgument(
            'heuristic_weight',
            default_value='2.0',  # Increased from 1.5 to 2.0 for faster planning with more greedy search
        ...
        Node(
            ...
            parameters=[{
                ...
                # TF publishing - ULTRA AGGRESSIVE
                'publish_tf': True,  # Explicitly enable TF publishing
                'publish_tf_rate': 200.0,  # ULTRA high rate (200Hz) for instant transform updates
                
                # IMU-specific parameters - optimized for map rotation
                'initial_yaw_offset': 0.0,
                'use_filtered_yaw': True,
                'yaw_filter_size': 2,          # Smaller filter for even faster response (was 3)
                'yaw_weight': 1.0,             # Maximum IMU weight (was 0.95)
        ...
        # Set up the pose generator for testing (simulates a goal pose)
        # COMMENTED OUT to prevent conflicts with test_navigation
        # goal_pose_publisher = Node(
        #     package='sensor_fusion',
        #     executable='test_goal_pose_publisher',
        #     name='goal_pose_publisher',
        #     parameters=[{
        #         'publish_rate': 1.0,  # How often to publish test goal poses (Hz)
        #         'map_frame_id': 'map',
        #         'random_goals': False,  # Whether to generate random goals or use predefined ones
        #         'goal_distance': 8.0,  # Distance from current position for test goals (if random)
        #     }],
